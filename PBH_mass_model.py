# -*- coding: utf-8 -*-
"""
Created on Sat Feb 27 13:39:35 2021
This python notebook is based on the Mathematica notebook used by B. Carr et al. in their paper 
"Cosmic Conundra Explained by Thermal History and Primordial Black Holes".
In this notebook, we infer the mass spectrum for a possible primordial black hole population from the thermal
history of the early Universe. We then take into account the sensitivity of the LIGO / Virgo detectors during
their O2 runs to see if we can detect gravitational waves coming from mergers of this population.  
...
#mPBHplot = np.geomspace(4 * 10**-1, 10**1, num = 250)
...
HighMASSrate = 0.6994      #From interp1d

#HighMASSrate = 0.6973       #From PCHIP
...
#Making interpolations of the O1 and O2 data using PCHIP
#ratelimitsO1 = interpolate.PchipInterpolator(O1[:,0], O1[:,1], extrapolate = 'yes')
#ratelimitsO2 = interpolate.PchipInterpolator(O2[:,0], O2[:,1], extrapolate = 'yes')
...
#Interpolating the logarithm of the noise data using PCHIP
#LIGOnoiseInterp = interpolate.PchipInterpolator(np.log10(LIGOnoise[:,0]), np.log10(LIGOnoise[:,1]), extrapolate = 'yes')
...
#Again, interpolating the noise data, now using PCHIP
#sqrtSfunctionInterp = interpolate.PchipInterpolator(np.log10(sqrtSfunction[:,0]), np.log10(sqrtSfunction[:,1]), extrapolate = 'yes')
...
integrant_prep = interpolate.splrep(integrant_list[:,0], integrant_list[:,1], k = 3, s = 0)

#Defining the functions needed for the 3D plots
#Doing the spline interpolation for the luminosity distance to the furthest detectable source
def integrant(x_spl):
...
logmA2 = np.linspace(-0.5, 2.5, num = 150)
logmB2 = np.linspace(-0.5, 2.5, num = 150)
...
#Showing some of the values of figure 3 of the paper
